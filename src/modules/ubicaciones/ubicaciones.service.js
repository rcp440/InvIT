...
const obtener = async (id, tenantId) => {
    const u = await repo.findById(id, tenantId);
    if (!u) throw Object.assign(new Error('Ubicación no encontrada'), { status: 404 });
    return u;
};

const crear    = async (tenantId, datos) => repo.create({ tenantId, ...datos });
const actualizar = async (id, tenantId, datos) => { await obtener(id, tenantId); return repo.update(id, tenantId, datos); };
const eliminar = async (id, tenantId) => {
    const r = await repo.remove(id, tenantId);
    if (!r) throw Object.assign(new Error('Ubicación no encontrada'), { status: 404 });
};
